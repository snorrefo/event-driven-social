...
class FollowRepositoryTest {
    ...
    @Test
    fun `should find follower ids`() {
        val followedId = UUID.randomUUID()
        val follower1 = UUID.randomUUID()
        val follower2 = UUID.randomUUID()

        followRepository.save(Follow(followerId = follower1, followedId = followedId))
        followRepository.save(Follow(followerId = follower2, followedId = followedId))

        val followerIds = followRepository.findFollowerIds(followedId)

        assertEquals(2, followerIds.size)
        assertTrue(followerIds.containsAll(listOf(follower1, follower2)))
    }
    ...
    fun `should return empty list when no followers`() {
        ...
        val followerIds = followRepository.findFollowerIds(userId)

        assertTrue(followerIds.isEmpty())
    }
