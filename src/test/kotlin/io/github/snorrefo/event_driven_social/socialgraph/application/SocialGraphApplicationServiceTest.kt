...
class SocialGraphApplicationServiceTest {
    ...
    private val service = SocialGraphApplicationService(
        followRepository, outboxRepository, eventSerializer
    )
    ...
    @Test
    fun `should get follower ids`() {
        val userId = UUID.randomUUID()
        val followerIds = listOf(UUID.randomUUID(), UUID.randomUUID())

        whenever(followRepository.findFollowerIds(userId)).thenReturn(followerIds)

        val result = service.getFollowerIds(userId)
