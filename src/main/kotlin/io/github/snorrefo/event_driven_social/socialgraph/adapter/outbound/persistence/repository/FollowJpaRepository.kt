...
interface FollowJpaRepository : JpaRepository<FollowJpaEntity, FollowId> {
    ...
    @Query("SELECT f.followerId FROM FollowJpaEntity f WHERE f.followedId = :followedId")
    fun findFollowerIdsByFollowedId(@Param("followedId") followedId: UUID): List<UUID>
